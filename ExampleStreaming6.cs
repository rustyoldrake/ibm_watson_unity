/**
demo here https://youtu.be/mrKzq-z7h14
Part 1
Implemented some basic LOGGING so i can start to do more ground truth capture with voice in my lab.  
Each utterance is captured and classified as mode/state.
Voice attributes of Text to Speech are changeable using Verbal command and Control.
*/
...
public class ExampleStreaming : MonoBehaviour
{
	...
	public int voicebox_prosody_pitch = 0;  // -100% to 100%
    ...
    void Start()
    {
		...
		_testString = "<speak version=\"1.0\"><express-as type=\"GoodNews\">Hello! Welcome to IBM Watson Unity Voicebox!</express-as></speak>";
		...
		voicebox_prosody_pitch = 150;  // 150 hZ to 350 hz
            ...
            foreach (var res in result.results)
            {
				foreach (var alt in res.alternatives) {
					...
					if (ResultsField.text.Contains ("Final")) // needs to be final or ECHO happens
					{
						...
						else if (alt.transcript.Contains ("voicebox"))
						{	
							_testString = "Voice traits have been modified.";

							if ( alt.transcript.Contains ("increase") && alt.transcript.Contains ("pitch") )
							{
								voicebox_prosody_pitch = voicebox_prosody_pitch + 25;  // 150 hZ to 350 hz
							...
							else if (alt.transcript.Contains ("decrease") && alt.transcript.Contains ("pitch") )
							{
								voicebox_prosody_pitch = voicebox_prosody_pitch - 25;  // 150 hZ to 350 hz
								voicebox_pitch_range = voicebox_pitch_range - 25;  // -100% to 100%
							...
							{
								voicebox_prosody_pitch = 0;  // -100% to 100%
								voicebox_pitch_range = 0;  // -100% to 100%
						...
						else if (alt.transcript.Contains ("status"))
						{
							...
							" utterances in the transcript log." +
						...
						//// Check if file exists (later, we dont need to do this all the time, but here for now.  OPTIMIZE LATER
						...
						//// WRITE TO LOG FILE - time-date and MODE and transcript (final)	
